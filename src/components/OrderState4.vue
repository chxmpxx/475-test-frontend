    ...
    <div v-if="status5" class="flex flex-wrap -mx-3 mb-10 mt-4">
      <div class="w-full md:w-2/4 px-3 md:mb-0">
        ...
        <div v-if="!status0" class="relative pb-4">
          ...
          >
            <option disabled>เฉพาะชุบเครื่องประดับเท่านั้น</option>
            <option
...
export default {
  ...
  data() {
    return {
      ...
      status0: false, // ถ้าชุบเครื่องประดับจะไม่มีให้เลือกชื่อพนักงาน
  ...
  async created() {
    if (this.o_info.status != "เสร็จสิ้นการขัดเครื่องประดับ") {
      ...
      for (var i = 0; i < this.ot_all.length; i++) {
        if (
          ...
          (this.ot_all[i].task_name == "ชุบเครื่องประดับ" ||
            this.ot_all[i].task_name == "เสร็จสิ้นการชุบเครื่องประดับ")
        ) {
      ...
      if (this.o_info.status == "ชุบเครื่องประดับ") {
        this.status3 = true;
      } else {
  ...
  methods: {
    ...
    async filter() {
      for (var i = 0; i < this.employee_info.length; i++) {
        if (this.employee_info[i].job == "ช่างชุบ") {
    ...
    status02() {
      ...
      if (
        ...
        Swal.fire({
          title: "ยืนยันการแจกงานชุบเครื่องประดับ",
          ...
          if (result.isConfirmed) {
            ...
            Swal.fire({
              title: "แจกงานชุบเครื่องประดับสำเร็จ!",
    ...
    good() {
      Swal.fire({
        title: "ยืนยันการชุบเครื่องประดับผ่าน",
        icon: "warning",
        showCancelButton: true,
        confirmButtonColor: "#3085d6",
        cancelButtonColor: "#d33",
        ...
        if (result.isConfirmed) {
          ...
          Swal.fire({
            title: "เสร็จสิ้นการชุบเครื่องประดับ!",
            icon: "success",
            confirmButtonColor: "#3085d6",
            confirmButtonText: "กลับ",
          }).then((result) => {
    ...
    bad() {
      Swal.fire({
        title: "ยืนยันการชุบเครื่องประดับไม่ผ่าน",
        icon: "warning",
        showCancelButton: true,
        confirmButtonColor: "#3085d6",
        cancelButtonColor: "#d33",
        ...
        if (result.isConfirmed) {
          ...
          Swal.fire({
            title: "ไม่ผ่านการชุบเครื่องประดับ!",
            icon: "error",
            confirmButtonColor: "#d33",
            confirmButtonText: "กลับ",
          }).then((result) => {
